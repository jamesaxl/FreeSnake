...
class Radio(object):
    ...
    def player_radio(self, path_file):
        if not Path(path_file).exists():
            raise FileNotFoundError('{0} does not exist'.format(path_file))
        
        instance = vlc.Instance()
        list_ = instance.media_list_new()
        
        urls = open(path_file, 'r')
        
        while True:
            url = urls.readline()
            if not url: 
                break
            list_.insert_media(instance.media_new('http://127.0.0.1:8888/{0}'.format(url)))
        
        player = instance.media_list_player_new()
        player.set_media_list(list_)
        return player
    ...
    def upload_radio_callback(self, event, result):        
        if event == 'URIGenerated':
            ...
            LOGGER.info('WEBSITE: {0} PROGRESS: {1}%'.format(self.name, round(progress, 2)))
    ...
    def sync_db_with_dir(self, identifier):
        ...
        track_files_in_db = self.track_model.select_belong_to_website(identifier)
        track_files_in_db = [_file[1] for _file in separate_files_in_db]
        track_files_in_dir = [_file['name'] for _file in self.separate.queue['data'] if _file['name'] is not None]
        track_files_to_delete_from_db = [_file for _file in separate_files_in_db if not _file in separate_files_in_dir ]
