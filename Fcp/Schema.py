...
class FromClientToNode(object):
    ...
    def reload_plugin(**kw):
        ...
        return reload_plg.encode('utf-8')
    ...
    def generate_keys():
        ...
        generate_key = 'GenerateSSK\n'
        generate_key += 'Identifier={0}\n'.format(identifier)
        generate_key += 'EndMessage\n'

        return generate_key.encode('utf-8'), identifier
    ...
    def put_data(compression_codecs, **kw):
        ...
        if extra_inserts_single_block is not None:
        ...
        if extra_inserts_splitfile_header_block is not None:
        ...
        if override_splitfile_crypto_key is not None:
    ...
    def put_file(node_identifier, compression_codecs, **kw):
        ...
        def sha256dda(node_identifier, identifier, path_of_file):
            ...
            """
            to_hash = b'-'.join(
                [node_identifier.encode('utf-8'), identifier.encode('utf-8'), open(path_of_file, "rb").read()])
        ...
        if not manifest_files:
            ...
            for index, _file in enumerate(files):
                ...
                put_directory_f += 'Files.{0}.Metadata.ContentType={1}\n'.format(index,
                                                                                 magic.from_file(_file.as_posix(),
                                                                                                 mime=True))
    ...
    def put_directory_disk(**kw):
        ...
        default_name = kw.get('default_name', None)
    ...
    def get_stream(stream, **kw):
        ...
        if not hasattr(stream, 'write'):
            raise Exception(
                'stream must be a file object with attribute write e.g: stream = open(\'filename.extension\', \'ab\')')
    ...
    def persistent_put_complex_dir_files(data):
        ...
        if data.get('Files.Count', False):
            for _index in range(int(data['Files.Count'])):
                schema_succ['Files.{0}.UploadFrom'.format(_index)] = {'type': 'string', 'required': True,
                                                                      'empty': False}
                schema_succ['Files.{0}.DataLength'.format(_index)] = {'type': 'string', 'required': True,
                                                                      'empty': False}
                ...
                schema_succ['Files.{0}.Metadata.ContentType'.format(_index)] = {'type': 'string', 'required': True,
                                                                                'empty': False}
...
def parsing_received_request_in_bytes(data):
